...
- [x] 每周复盘策略表现和参数漂移：已通过 backtestreport、walkforward、dailyreport 提供输入。
...
- [x] 合约小资金实盘必须从 1x 开始，单合约名义仓位不超过账户权益的 25%：规则已写入风控/文档，真实执行待审批。
...
- [x] 完成合约 1x paper trading，验证资金费率、保证金和强平距离日志：本地字段和命令入口已完成，真实连续验证待执行。
