...
> v3 草案：前期小资金允许使用 `aggressive` 档做更主动的合约趋势短线，但仍必须保留强平距离、单笔风险、保证金占用和交易频率约束。
...
| 追价距离过滤 | 可选 `max_entry_extension_pct` | 价格离快均线太远时不追单 |
| 回踩确认 | 可选 `pullback_lookback` / `pullback_tolerance_pct` | 顺趋势开仓前，最近 N 根 K 线必须回踩或反抽到快均线附近 |
| 杠杆 | `1.0x` | 当前不是高杠杆 |
...
2. 最新收盘价 > 上一根收盘价。
3. 如果启用趋势强度、ATR、成交量过滤，必须全部通过。
4. 如果启用追价距离过滤，当前价不能离快均线太远。
5. 如果启用回踩确认，最近 N 根 K 线必须曾经回踩到快均线附近。
6. 当前不在冷却期。
...
3. 如果启用趋势强度、ATR、成交量过滤，必须全部通过。
4. 如果启用追价距离过滤，当前价不能离快均线太远。
5. 如果启用回踩确认，最近 N 根 K 线必须曾经反抽到快均线附近。
6. 当前不在冷却期。
...
前期资金量很小时，可以允许更激进的 paper 参数做探索，但这个模式只能用于小资金灰度和策略研究，不能直接作为多人跟单默认档。

当前新增 `-profile aggressive`，含义是：

| 参数 | aggressive 值 | 设计目的 |
| --- | --- | --- |
| 市场 | `perpetual` | 只测合约，多空都允许 |
| 周期 | `1m` | 保持短线频率 |
| 均线 | `fast=3` / `slow=12` | 比 3/9 稍微降噪，但仍然敏感 |
| 止盈/止损 | `0.65%` / `0.25%` | 扩大净止盈空间，避免手续费吃掉大部分利润 |
| 杠杆 | `3x` | 小资金提高资金利用率 |
| 单笔风险 | `risk_pct=2%` | 比保守档更激进，但仍按止损距离反推数量 |
| 单笔风险硬上限 | `max_order_risk_pct=2.5%` | 防止参数错误导致单笔亏损失控 |
| 名义仓位上限 | `max_notional_pct=220%` | 允许接近 2.2 倍权益的名义敞口 |
| 保证金上限 | `max_margin_pct=65%` | 防止保证金被单策略吃满 |
| 可用余额占用 | `max_balance_use_pct=90%` | 小资金阶段可以更主动使用余额 |
| 强平距离 | `min_liquidation_distance_pct=15%` | 杠杆提高后必须留安全距离 |
| 趋势强度 | `min_trend_spread_pct=0.03%` | 过滤几乎没有方向的均线缠绕 |
| ATR 过滤 | `atr_window=14` / `0.08%-1.6%` | 波动太小不做，波动异常大不追 |
| 成交量过滤 | `volume_window=20` / `min_volume_ratio=1.15` | 尽量只做放量方向 |
| 追价过滤 | `max_entry_extension_pct=0.18%` | 离快均线太远不追 |
| 回踩确认 | `pullback_lookback=5` / `pullback_tolerance_pct=0.06%` | 顺趋势前先要求最近回踩/反抽 |
...
手动传入的参数优先级高于 profile。例如 `-profile aggressive -leverage 2` 会保留 `2x`，不会被 profile 覆盖成 `3x`。

这个模式必须遵守以下降档条件：

1. 单日净亏损达到 `4%`，当天停止 aggressive 档。
2. 连续亏损 `3` 笔，暂停至少 30 分钟。
3. 最大回撤超过 `8%`，切回 `1x-2x` 和 `risk_pct <= 1%`。
4. 连续 paper trading 少于 2 周，不允许接真实跟单资金。
5. 一旦进入多人跟单，默认不能使用 aggressive 档，必须按每个跟单账户独立降杠杆、降风险。
...
普通 paper 档仍建议保持 `1x` 到 `2x`。小资金研究阶段可以用 `aggressive` 档测试 `3x`，但必须同时打开强平距离、保证金占用、单笔风险和连续亏损暂停。进入真实交易或多人跟单前，应优先降到 `1x-2x`，并使用交易所返回的 leverage bracket、账户模式、reduce-only 平仓和异常熔断。
...
| 小资金激进 | 3 | 12 | 0.65% | 0.25% | 只适合小资金 paper/灰度，配合 `3x`、`risk_pct=2%` 和回踩过滤 |
...
8. 改固定 TP/SL 为 ATR 或半动态 TP/SL。
9. 增加分批止盈和保本止损。
10. 增加连续亏损暂停和日亏损暂停。
11. 做参数网格回测和 walk-forward 验证。
12. 连续 paper trading 至少 2 到 4 周，再考虑小资金真实灰度。
