...
| 周期 | 默认 `5m`，`aggressive` 档 `3m` | 降低 1m 噪音；仍可用 `-interval` 手动覆盖 |
...
| 风险仓位 | 默认 `risk_pct=1` | 按单笔止损风险占账户权益比例反推数量；`aggressive` 档为 `2` |
...
| 杠杆 | 默认 `1.0x`，`aggressive` 档 `3.0x` | 真实下单仍默认关闭 |
| 执行价格 | OneBullEx `mark price` 优先 | K 线 close 只负责出信号；默认要求存在 mark price |
| K 线新鲜度 | `max_candle_age=7m` | 适配默认 5m 信号周期；旧参数 `max_market_data_age` 仍可覆盖 |
...
| 信号评分过滤 | `signal_filter=true` / `min_signal_score=0.55` | `aggressive` 档阈值为 `0.50`；低分候选开仓转为观望 |
...
- 读取本地真实 OneBullEx 合约 5m K 线；`aggressive` 档使用 3m。
- paper 策略执行价优先读取 OneBullEx mark price；K 线 close 只负责信号判断。
- 对最新 K 线 close 和 mark price 分别做新鲜度检查，避免 5m 信号周期被 mark price 阈值误杀。
...
- 看板默认展示合约 `BTCUSDT`，策略默认信号周期为 `5m`，并过滤掉现货交易噪音。
...
- `cmd/papertrade/main.go:27`：paper 默认参数，包括 `5m` 周期、动态 TP/SL、风险仓位和行情新鲜度。
...
| 周期 | `3m` | 比 1m 降噪，但仍保留小资金短线频率 |
| 均线 | `fast=3` / `slow=9` | 保持短线趋势敏感度 |
| 止盈/止损 | ATR 动态 | 止盈 `ATR*1.8`，区间 `0.60%~1.60%`；止损 `ATR*1.0`，区间 `0.30%~0.75%` |
| 杠杆 | `3x` | 小资金提高资金利用率 |
...
| 多周期趋势过滤 | 3m/5m 开仓方向必须和 15m 趋势一致 |
...
| --- | --- | --- | --- | --- | --- |
| 当前默认 | 5m | 3 | 9 | ATR `1.6R/1.0R`，TP `0.55%~1.40%`，SL `0.30%~0.75%` | 降噪短线趋势 |
| 小资金激进 | 3m | 3 | 9 | ATR `1.8R/1.0R`，TP `0.60%~1.60%`，SL `0.30%~0.75%` | 只适合小资金 paper/灰度，配合 `3x`、`risk_pct=2%` |
| 更稳版本 | 5m | 5 | 20 | ATR `1.5R/1.0R` | 减少噪音交易 |
| 趋势版本 | 15m | 5 | 20 | ATR `1.8R/1.0R` | 更少交易，持仓更久 |
| 快速试错 | 3m | 3 | 12 | ATR `1.6R/1.0R` | 保留短线频率，但比 1m 稳 |
...
1. 先同步至少 6 到 12 个月 `5m` K 线，必要时再同步 `3m` 和 `15m`。
...
2. 默认低于 `0.55` 的候选开仓跳过，`aggressive` 档低于 `0.50` 跳过。
3. 不做反向交易，只把低分信号改成观望。
